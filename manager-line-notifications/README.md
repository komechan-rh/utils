...
- 給与未払いリマインド: 前月（支払い予定日ベース）の支払い状況が「未済」のままの場合、毎朝10:00 に LINE グループへリマインド通知します（支払い済みなら送信しません）。通知には、このグループで「給与支払い済」と発言すると支払い状況を更新できる旨を添えます。
...
- 毎朝10:00 に、前月分の給与支払い状況が「未済」であれば push メッセージでリマインド通知を送信（`src/index.ts` の `unpaidPayrollReminderToLine`）
...
- LINE グループ内で「給与支払い済」と発言すると、支払い状況を「済」に更新（未済のまま残っている前月分があればそちらを優先して更新し、なければ当月分を更新する）
